...
public interface SystemTypes
{
  ...
  public static List closureTaskdependsOn(List r)
  { Vector path = new Vector();
    for (int i = 0; i < r.size(); i++)
    { Task taskx = (Task) r.get(i);
      closureTaskdependsOn(taskx,path);
    }
    return path;
  }

  private static void closureTaskdependsOn(Task taskx, List path)
  { if (path.contains(taskx)) { return; }
    path.add(taskx);
    List dependsOnx = taskx.getdependsOn();
    for (int i = 0; i < dependsOnx.size(); i++)
    { Task taskxx = (Task) dependsOnx.get(i);
      closureTaskdependsOn(taskxx,path);
    }
  }
